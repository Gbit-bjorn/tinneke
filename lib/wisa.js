...
function decodeerResponse(result) {
  ...
  if (typeof raw === 'string') {
    ...
    try {
      ...
      if (buf.length > 0 && /[\x20-\x7E]/.test(buf.toString('ascii').slice(0, 20))) {
        // WISA geeft Latin-1 terug; probeer utf-8, val terug op latin1
        const utf8Tekst = buf.toString('utf-8');
        if (!utf8Tekst.includes('\uFFFD')) return utf8Tekst;
