 ...
 */
async function fetchAll(baseUrl, cache) {
  let url = baseUrl;
  const allResults = [];

  while (url) {
    // Cache op volledige URL
    let page;
    if (cache.has(url)) {
      page = cache.get(url);
    } else {
...
class LLinkidClient {
  ...
  constructor(baseUrl = DEFAULT_BASE) {
    ...
    /** @type {Map<string, any>} In-memory cache: URL → response */
    this._cache = new Map();
  }
  ...
  async getLeerplannen(zoekterm = null) {
    ...
    const items = await fetchAll(url, this._cache);
