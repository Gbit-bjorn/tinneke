...
class Database {
  ...
  async syncWisaKlassenLeerlingen(rijen, schooljaar) {
    ...
    const kolNaam     = vindKolom(headers, ['naam', 'familienaam', 'achternaam', 'last_name']);
    const kolVoornaam = vindKolom(headers, ['voornaam', 'first_name', 'firstname']);
    const kolKlas     = vindKolom(headers, ['klas_naam', 'klas', 'klasnaam', 'admin_groep_naam', 'groep', 'klsnm']);
    const kolRichting = vindKolom(headers, ['richting', 'studierichting', 'studierichting_naam', 'opleiding']);
    ...
    return { klassen: aantalKlassen, leerlingen: aantalLeerlingen };
  }
